...
export default async function Home() {
    ...
    const latestVideoId = "t96q-B-F1rU"; // 固定または今後microCMS化

    // スライダー用の画像配列（以前の形式に基づき、最新の3枚をセット）
    const slides = [
    ...
    return (
        <div className="bg-[#0a0a0a] min-h-screen text-white">
            ...
            <section className="relative h-screen w-full flex flex-col items-center justify-center overflow-hidden">
                ...
                {/* ヘッダーロゴ (PROFILEと同じデータを使用) */}
                <div className="absolute top-12 left-8 z-20">
                    {profile?.band_logo?.url && (
                        <img src={profile.band_logo.url} alt="ashroom" className="h-12 md:h-16 w-auto opacity-90" />
                    )}
                </div>
                ...
                <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {slides.map((slide, i) => (
                        <Link key={i} href={slide.link} className="group relative aspect-[4/5] overflow-hidden border border-white/5 bg-zinc-900">
                            ...
                            <div className="absolute top-4 left-4 border border-white/20 bg-black/60 text-[9px] tracking-widest px-2 py-1 uppercase">
                                {slide.label}
                            </div>
                ...
                <section>
                    ...
                    <div className="divide-y divide-white/5">
                        {newsData.contents.map((item) => {
                            ...
                            return (
                                <Link key={item.id} href={`/news/${item.id}`} className="group block py-10 hover:bg-white/[0.02] transition-all px-4 border-white/5">
