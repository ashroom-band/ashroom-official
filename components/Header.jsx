...
export default function Header({ profile }) {
    ...
    return (
        <header className="fixed top-0 w-full z-[100] bg-black/85 backdrop-blur-xl border-b border-white/5">
            <div className="w-full flex items-center justify-between h-20 md:h-24 px-4 md:px-6 lg:px-8">
                
                {/* 左端：ロゴ */}
                <Link href="/#top" className="relative z-[110] hover:opacity-70 transition-opacity flex items-center">
                ...
                <nav className="hidden lg:flex items-center gap-12">
                    <div className="flex items-center gap-10 text-sm md:text-base font-bold tracking-[0.2em] shippori-mincho">
                        {menuItems.map((item) => (
                    ...
                    <div className="flex items-center gap-6 pl-10 border-l border-white/20">
                        {snsLinks.map((sns) => (
                            <a key={sns.name} href={sns.url} target="_blank" rel="noopener noreferrer" className="w-5 h-5 fill-current hover:text-white/50 transition-colors flex items-center justify-center">
                                <svg viewBox="0 0 24 24" className="w-full h-full">{sns.icon}</svg>
                            </a>
                        ))}
                    </div>
                </nav>
                ...
                <button 
                    ...
                    className="lg:hidden relative z-[110] w-10 h-10 flex flex-col justify-center items-center"
                    aria-label="Menu"
                >
            ...
            {/* フルスクリーンメニュー */}
            <div className={`fixed inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center transition-all duration-500 ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
                <nav className="flex flex-col items-center gap-10 text-2xl font-bold tracking-[0.3em] shippori-mincho">
